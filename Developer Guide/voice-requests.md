...
[insert code sample: “Are you sure?” prompt]
...
[insert code sample: "How would you like to pay?" prompt with aliased options: "Cash[dollars, bills]; Credit[Visa, card, charge]"]
...
[insert code sample: "What is your destination address?" prompt]

### NumericalRequest

Similarly, to prompt the user for a strictly numerical response (e.g. "one", "third"), use the `NumericalRequest` class. This request is designed to handle both number (e.g. "two", "one-half") and ordinal (e.g. "first", "fourth") responses. The result of a `NumericalRequest` is an `NSNumber` instance.

[insert code sample: "What is the zip code?" prompt]
...
[insert code sample: "What move would you like to make?" chess game prompt with valid chess move patterns]
