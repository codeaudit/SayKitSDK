...
Each of these stages can be customized as the developer sees fit. For the particulars, check out the [header documentation for the `SAYVoiceRequest` protocol](#).
...
The final item here is the key to creating a turn-taking dialogue. By providing a followup requests, a request session turns into a cycle, with the app and user taking turns speaking to each other.
